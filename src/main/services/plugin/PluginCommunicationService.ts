...
import type { PluginMessage, PluginResponse } from './PluginCommunicationService.d'
...
export class PluginCommunicationService {
  ...
  private setupIPCHandlers(): void {
    ...
    ipcMain.handle(
      ...
      async (_event, pluginId: string, action: string, handler: Function) => {
    ...
    // 监听插件注销消息处理器
    ipcMain.handle(
      'plugin-unregister-handler',
      async (_event, pluginId: string, action?: string) => {
