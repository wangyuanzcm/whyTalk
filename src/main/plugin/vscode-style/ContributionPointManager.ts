...
export class ContributionPointManager extends EventEmitter {
  ...
  public registerContributions(extensionId: string, contributes: ExtensionContributes): Disposable {
    ...
    try {
      ...
      console.log(`[ContributionPointManager] 已注册扩展 ${extensionId} 的 ${contributionIds.size} 个贡献点`)
   ...
   */
  public getCommand(commandId: string): (CommandContribution & { extensionId: string }) | undefined {
   ...
   * @param configId 配置ID
   */
  public getConfiguration(configId: string): (ConfigurationContribution & { extensionId: string }) | undefined {
  ...
  public getExtensionContributions(extensionId: string): {
    ...
    return {
      ...
      configurations: Array.from(this.configurations.values()).filter(c => c.extensionId === extensionId),
      menus: Array.from(this.menus.values()).filter(m => m.extensionId === extensionId),
      keybindings: Array.from(this.keybindings.values()).filter(k => k.extensionId === extensionId),
