...
export function getThemePreviewColors(theme: Theme): {
  ...
  // 安全检查，确保主题对象和配置存在
  if (!theme || !theme.config || !theme.config.themeColors || !theme.config.extInfo) {
    return {
      primary: '#007bff',
      background: '#ffffff',
      text: '#333333',
  ...
  return {
    primary: theme.config.themeColors['--color-primary'] || '#007bff',
    background: theme.config.extInfo['--color-main-background'] || '#ffffff',
    text: theme.config.themeColors['--color-1000'] || '#333333',
    accent: theme.config.themeColors['--color-theme'] || '#6c757d'
  }
}