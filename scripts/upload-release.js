...
async function uploadFileToMinio(filePath, retryCount = 0) {
  try {
    ...
    console.log(`📤 开始上传到 MinIO: ${fileName} (${(fileSize / 1024 / 1024).toFixed(2)} MB)${retryText}`);
    ...
    const objectName = minioConfig.pathPrefix ? 
      path.posix.join(minioConfig.pathPrefix, fileName) : fileName;
...
async function uploadFileToHttp(filePath, retryCount = 0) {
  try {
    ...
    const retryText = retryCount > 0 ? ` (重试 ${retryCount}/${CONFIG.behavior.retryCount})` : '';
    console.log(`📤 开始上传到 HTTP: ${fileName} (${(fileSize / 1024 / 1024).toFixed(2)} MB)${retryText}`);
...
async function main() {
  ...
  if (serverType === 'minio') {
    const minioConfig = CONFIG.server.minio;
    console.log('MinIO 服务器:', `${minioConfig.useSSL ? 'https' : 'http'}://${minioConfig.endPoint}:${minioConfig.port}`);
