...
const startTag = new ExternalTokenizer(
  (input, stack) => {
...
function findLocation(state, pos) {
  ...
  if (inTag && (inTag.to > pos || inTag.lastChild.type.isError)) {
    ...
    if (
      (before === null || before === void 0 ? void 0 : before.name) == 'StartCloseTag' &&
      before.to <= pos
    )
...
class Element {
  constructor(spec, attrs, attrValues) {
    ...
    this.children = []
    this.name = spec.name
    this.completion = Object.assign(Object.assign({ type: 'type' }, spec.completion || {}), {
      label: this.name
    })
...
function attrCompletion(spec) {
  return Object.assign(Object.assign({ type: 'property' }, spec.completion || {}), {
    label: spec.name
  })
}
...
function completeFromSchema(eltSpecs, attrSpecs) {
  ...
  return (cx) => {
    ...
    if (type == 'openTag') {
        ...
        ? {
            ...
            options: [
              ((_a = byName[parentName]) === null || _a === void 0
                ? void 0
                : _a.closeNameCompletion) || { label: parentName + '>', type: 'type' }
            ],
            validFor: Identifier
          }
        : null
      ...
      if (!attr) return null
      let parent = byName[tagName(doc, context)]
      let values = ((parent === null || parent === void 0 ? void 0 : parent.attrValues) ||
        attrValues)[attr]
      ...
      let options = closing.concat(
        (
          ...
          (context ? allElements : topElements)
        ).map((e) => e.openCompletion)
      )
  ...
  (view, from, to, text, insertTransaction) => {
    ...
    let closeTags = state.changeByRange((range) => {
      ...
      if (didType && text == '>' && after.name == 'EndTag') {
        ...
        if (
          ((_b = (_a = tag.parent) === null || _a === void 0 ? void 0 : _a.lastChild) === null ||
          _b === void 0
            ? void 0
            : _b.name) != 'CloseTag' &&
        ...
        if (
          after.from == head - 2 &&
          ((_c = base2.lastChild) === null || _c === void 0 ? void 0 : _c.name) != 'CloseTag' &&
          (name = elementName(state.doc, base2, head))
        ) {
          let to2 = head + (state.doc.sliceString(head, head + 1) === '>' ? 1 : 0)
          let insert = `${name}>`
          return {
