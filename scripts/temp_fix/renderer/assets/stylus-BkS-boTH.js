...
function popAndPass(type2, stream, state, n) {
  for (var i = 1; i > 0; i--) state.context = state.context.prev
...
function typeIsBlock(type2, stream) {
  return (
    (endOfLine(stream) &&
      (type2 == '{' || type2 == ']' || type2 == 'hash' || type2 == 'qualifier')) ||
    type2 == 'block-mixin'
  )
}
...
states.block = function (type2, stream, state) {
  if (
    (type2 == 'comment' && startOfLine(stream)) ||
    (type2 == ',' && endOfLine(stream)) ||
    type2 == 'mixin'
  ) {
...
states.parens = function (type2, stream, state) {
  ...
  if (type2 == ')') {
    ...
    if (
      ...
      (!stream.string.match(/^-?[a-z][\w-\.\[\]\'\"]*\s*=/) && wordIsTag(firstWordOfLine(stream)))
    ) {
      return pushContext(state, stream, 'block')
    }
    if (
...
const stylus = {
  ...
  indent: function (state, textAfter, iCx) {
    var cx = state.context,
      ch2 = textAfter && textAfter.charAt(0),
      indent = cx.indent,
    ...
    if (
      cx.prev &&
      ((ch2 == '}' && (cx.type == 'block' || cx.type == 'atBlock' || cx.type == 'keyframes')) ||
        (ch2 == ')' && (cx.type == 'parens' || cx.type == 'atBlock_parens')) ||
        (ch2 == '{' && cx.type == 'at'))
    ) {
