...
const indentation = new ContextTracker({
  start: Context.top,
  reduce(context, term) {
    return context.type == type_Flow && (term == FlowSequence || term == FlowMapping)
      ? context.parent
      : context
  },
    ...
    if ((isBreakSpace(prev) || prev < 0) && stack.context.type != type_Flow) {
      ...
      if (
        (depth < stack.context.depth ||
          (depth == stack.context.depth &&
            stack.context.type == type_Seq &&
            (input.next != 45 || !isSep(input.peek(1))))) && // Not blank
        input.next != -1 &&
...
const blockMark = new ExternalTokenizer(
  (input, stack) => {
    ...
    if (input.next == 45) {
      ...
      if (isSep(input.next))
        input.acceptToken(
          stack.context.type == type_Seq && stack.context.depth == findColumn(input, input.pos - 1)
      ...
      if (isSep(input.next))
        input.acceptToken(
          stack.context.type == type_Map && stack.context.depth == findColumn(input, input.pos - 1)
      ...
      if (input.next == 58) {
        ...
        if (isSep(after))
          input.acceptTokenTo(
            stack.context.type == type_Map && stack.context.depth == findColumn(input, start)
              ? mapContinueMark
              : mapStartMark,
            start
          )
...
function readPlain(input, scan, inFlow, indent) {
  if (
    charTag(input.next) == 's' ||
    ((input.next == 63 || input.next == 58 || input.next == 45) && isSafe(input.peek(1), inFlow))
  ) {
    input.advance()
  } else {
    return false
...
const blockLiteral = new ExternalTokenizer((input, stack) => {
  let indent = stack.context.type == type_Lit ? stack.context.depth : -1,
    upto = input.pos
  scan: for (;;) {
    let depth = 0,
      next = input.next
  ...
  parser: /* @__PURE__ */ parser$1.configure({
    props: [
      /* @__PURE__ */ indentNodeProp.add({
        Stream: (cx) => {
          for (
            let before = cx.node.resolve(cx.pos, -1);
