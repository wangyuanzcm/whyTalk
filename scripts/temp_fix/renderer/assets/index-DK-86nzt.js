import {
  ...
  f as foldNodeProp,
  t as tags,
  j as delimitedIndent,
  d as ExternalTokenizer
} from './index-iKnyGKVZ.js'
import './index-CP-MMhae.js'
...
const parser = /* @__PURE__ */ LRParser.deserialize({
  ...
  repeatNodeCount: 11,
  tokenData:
    ")Q~RkXY!vYZ!v]^!vpq!vqr#Xrs#duv$Uwx$axy$|yz%R{|%W|}&r}!O&w!O!P'T!Q![&a![!]'e!^!_'j!_!`'r!`!a'j!c!}'z#R#S'z#T#o'z#p#q(p#q#r(u%W;'S'z;'S;:j(j<%lO'z~!{S%O~XY!vYZ!v]^!vpq!v~#[P!_!`#_~#dOa~~#gUOY#dZr#drs#ys;'S#d;'S;=`$O<%lO#d~$OOn~~$RP;=`<%l#d~$XP#q#r$[~$aOv~~$dUOY$aZw$awx#yx;'S$a;'S;=`$v<%lO$a~$yP;=`<%l$a~%ROf~~%WOe~P%ZQ!O!P%a!Q![&aP%dP!Q![%gP%lRoP!Q![%g!g!h%u#X#Y%uP%xR{|&R}!O&R!Q![&XP&UP!Q![&XP&^PoP!Q![&XP&fSoP!O!P%a!Q![&a!g!h%u#X#Y%u~&wO!^~~&zRuv$U!O!P%a!Q![&a~'YQ]S!O!P'`!Q![%g~'eOh~~'jOs~~'oPa~!_!`#_~'wPd~!_!`#__(TV^WuQ%RT!Q!['z!c!}'z#R#S'z#T#o'z%W;'S'z;'S;:j(j<%lO'z_(mP;=`<%l'z~(uOq~~(xP#q#r({~)QOX~",
  tokenizers: [base, raw, comment, inlineComment, 0, 1, 2, 3],
  topRules: { Template: [0, 7] },
  specialized: [
    { term: 187, get: (value) => spec_identifier[value] || -1 },
    { term: 37, get: (value) => spec_TagName[value] || -1 }
  ],
  tokenPrec: 0
})
...
const Filters = /* @__PURE__ */ completions(
...
const Tags = /* @__PURE__ */ completions(
...
const Expressions = /* @__PURE__ */ completions(
  'empty forloop tablerowloop in with as contains',
  'keyword'
)
const forloop = /* @__PURE__ */ completions('first index index0 last length rindex', 'property')
const tablerowloop = /* @__PURE__ */ completions(
...
function findContext(context) {
  ...
  let before =
    ((_a = node.childBefore(pos)) === null || _a === void 0 ? void 0 : _a.name) || node.name
  ...
  if (node.name == 'MemberExpression' && before == '.') return { type: 'property', target: node }
  if (node.name == 'VariableName') return { type: 'expression', from: node.from }
...
function liquidCompletionSource(config = {}) {
  ...
  return (context) => {
    ...
    else options = resolveProperties(context.state, cx.target, context, properties)
...
const tagLanguage = /* @__PURE__ */ LRLanguage.define({
  ...
  parser: /* @__PURE__ */ parser.configure({
    props: [
      /* @__PURE__ */ styleTags({
        'cycle comment endcomment raw endraw echo increment decrement liquid in with as':
          tags.keyword,
      ...
      /* @__PURE__ */ indentNodeProp.add({
        ...
        'UnlessDirective ForDirective TablerowDirective CaptureDirective':
          /* @__PURE__ */ directiveIndent(/^\s*(\{%-?\s*)?end\w/),
