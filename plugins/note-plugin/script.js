...
class NotePlugin {
    ...
    async loadNotes() {
        try {
            ...
            if (response.success) {
                this.notes = response.data.filter(item => item.key.startsWith('note_')).map(item => ({
    ...
    async saveNotes() {
        try {
            ...
            for (const note of this.notes) {
                const noteData = { ...note }
                delete noteData.id
                await window.electronAPI.invoke('plugin-data-set', 
                    this.pluginId, 
                    `note_${note.id}`, 
    ...
    async loadSharedNotes() {
        try {
            // 从共享数据命名空间加载笔记
            const response = await window.electronAPI.invoke('plugin-shared-data-list', 
                this.dataNamespace, this.pluginId)
            if (response.success) {
    ...
    async shareCurrentNote() {
        ...
        try {
            ...
            await window.electronAPI.invoke('plugin-shared-data-set',
                this.dataNamespace,
                sharedKey,
