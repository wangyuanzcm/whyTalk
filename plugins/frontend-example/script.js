...
if (typeof window.pluginAPI === 'undefined') {
    ...
    document.body.innerHTML = '<div style="padding: 20px; text-align: center; color: red;">插件API不可用，请确保在Why-Talk插件环境中运行</div>';
} else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (messageInput) {
        messageInput.value = messageInput.value.replace('${new Date().toISOString()}', new Date().toISOString());
    }
});
