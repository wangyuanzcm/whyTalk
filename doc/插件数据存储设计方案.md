...
export class PluginDataService {
  ...
  // 消息数据操作
  async getMessages(conversationId: number, page: number, limit: number, pluginId: string): Promise<Array<any>>
  ...
  async pinConversation(conversationId: number, isPinned: boolean, pluginId: string): Promise<boolean>
  ...
  async updateFileTransferStatus(transferId: number, status: string, pluginId: string): Promise<boolean>
}
```
...
export class PluginDataService {
  ...
  // 批量更新消息状态
  async batchUpdateMessageStatus(messageIds: number[], status: number, pluginId: string): Promise<boolean> {
