...
} from '../../shared/services/DataSyncService'
...
export default {
  ...
  methods: {
    ...
    async sendMessage(content: string, messageType: string = 'text', attachments?: any[]) {
      ...
      try {
        const messageData = {
          ...
          receiver_id:
            this.conversation.conversation_type === 'private'
              ? this.conversation.participants?.find(
                  (p) => p.user_id !== this.$store.state.user.userId
                )?.user_id
              : undefined,
          content,
    ...
    async deleteMessage(message: MessageWithDetails) {
      try {
        const confirmed = await this.$confirm('确定要删除这条消息吗？', '确认删除', {
          type: 'warning'
        })
...
const batchUpdateContacts = async (updates: Array<{ id: number; data: any }>) => {
  ...
  for (const batch of batches) {
    await Promise.all(batch.map((update) => contactService.updateContact(update.id, update.data)))
  }
}
```
