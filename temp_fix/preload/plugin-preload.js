...
const pluginAPI = {
  ...
  system: {
    async getInfo() {
      const hasPermission = await electron.ipcRenderer.invoke("plugin:permission:check", "system:info");
      if (!hasPermission) {
  ...
  files: {
    async readText(path) {
      const hasPermission = await electron.ipcRenderer.invoke("plugin:permission:check", "filesystem:read");
      if (!hasPermission) {
    ...
    async writeText(path, content) {
      const hasPermission = await electron.ipcRenderer.invoke("plugin:permission:check", "filesystem:write");
      if (!hasPermission) {
  ...
  network: {
    async fetch(url, options) {
      const hasPermission = await electron.ipcRenderer.invoke("plugin:permission:check", "network:access");
      if (!hasPermission) {
  ...
  clipboard: {
    async readText() {
      const hasPermission = await electron.ipcRenderer.invoke("plugin:permission:check", "clipboard");
      if (!hasPermission) {
  ...
  systemPlugins: {
    async execute(pluginId, functionName, input) {
      try {
        const result = await electron.ipcRenderer.invoke("plugin:execute", pluginId, functionName, input);
        return result;
      } catch (error) {
  ...
  ui: {
    ...
    async executeAction(pluginId, actionName, params) {
      try {
        const result = await electron.ipcRenderer.invoke("plugin:ui:execute-action", pluginId, actionName, params);
        if (result.success) {
