...
const trackIndent = new ContextTracker({
  ...
  reduce(context, term, _, input) {
    if (
      (context.flags & cx_Bracketed && bracketed.has(term)) ||
...
const strings = new ExternalTokenizer((input, stack) => {
  ...
  for (;;) {
    if (input.next < 0) {
      ...
      input.next == quote &&
      (!long || (input.peek(1) == quote && input.peek(2) == quote))
    ) {
      if (input.pos == start) {
...
function skipEscape(input, ch) {
  if (ch == letter_o) {
    ...
    if (input.next == braceOpen) {
      ...
      while (
        input.next >= 0 &&
        input.next != braceClose &&
        input.next != singleQuote &&
        input.next != doubleQuote &&
        input.next != newline
      )
        input.advance()
  ...
  ImportStatement(_node, def) {
    ...
    for (let ch = node.getChild('import'); ch; ch = ch.nextSibling) {
      if (
        ...
        ((_b = ch.nextSibling) === null || _b === void 0 ? void 0 : _b.name) != 'as'
      )
    ...
    props: [
      /* @__PURE__ */ indentNodeProp.add({
        ...
        MatchBody: (context) => {
          ...
          return (_a = indentBody(context, inner || context.node)) !== null && _a !== void 0
            ? _a
            : context.continue()
        },
        IfStatement: (cx) =>
        ...
        TryStatement: (cx) =>
          /^\s*(except[ :]|finally:|else:)/.test(cx.textAfter) ? cx.baseIndent : cx.continue(),
        MatchStatement: (cx) => {
        ...
        Script: (context) => {
          var _a
          let inner = innerBody(context)
          return (_a = inner && indentBody(context, inner)) !== null && _a !== void 0
            ? _a
            : context.continue()
        }
      }),
